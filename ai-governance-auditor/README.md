...
2. 2. **Was it escalated appropriately given its stated priority or risk level?**
   3. 3. **Is there anything that couldn't be defended if reviewed later?**
     
      4. It returns a verdict — Pass, Needs Review, or Flagged — with specific findings, not just a score.
     
      5. ## Where the thinking comes from
     
      6. I'm an Archer Certified Administrator, and GRC frameworks like Archer are built around one core idea: nothing runs on trust alone — every risk and control needs an evidence trail. This project applies that same standard to AI decision logs instead of compliance controls.
     
      7. ## How it was built
     
      8. - **Logic and evaluation criteria:** designed by me, based on Archer's control-review approach
