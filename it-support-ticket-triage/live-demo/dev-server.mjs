// dev-server.mjs
// Local preview server — runs the triage demo on your own machine without
// installing the Vercel CLI. It serves ./public and routes POST /api/classify to
// the same api/classify.js handler Vercel runs in production, wrapping req/res so
...
//
// Production deploys do not use this file — Vercel invokes api/classify.js directly.
...
server.listen(PORT, HOST, () => {
  const mode = process.env.ANTHROPIC_API_KEY
    ...
    : "DEMO MODE (no ANTHROPIC_API_KEY — replies are stubbed, retrieval is real)";
  console.log(`Ticket Triage (RAG) running on http://${HOST}:${PORT}`);
